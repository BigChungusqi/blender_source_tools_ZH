...
class ValveSource_SceneProps(PropertyGroup):
	...
	export_format : EnumProperty(name=get_id("export_format"),items=( ('SMD', "SMD", get_id("export_format_smd_desc") ), ('DMX', "DMX", get_id("export_format_dmx_desc") ) ),default='DMX')
	...
	use_kv2 : BoolProperty(name=get_id("kv2_name"),description=get_id("kv2_tip"),default=False)
	...
	smd_format : EnumProperty(name=get_id("smd_format"), items=(('SOURCE', "Source", get_id("smd_format_source_desc") ) , ("GOLDSOURCE", "GoldSrc", get_id("smd_format_goldsrc_desc") )), default="SOURCE")

class ValveSource_VertexAnimation(PropertyGroup):
	name : StringProperty(name=get_id("vertexanim_name"),default="VertexAnim")
	start : IntProperty(name=get_id("vertexanim_start"),description=get_id("vca_start_tip"),default=0)
	end : IntProperty(name=get_id("vertexanim_end"),description=get_id("vca_end_tip"),default=250)
...
def register():
	...
	for _enc in datamodel.list_support()['binary']:
		translations._data[f"dmx_encoding_binary_{_enc}"] = {'en': f"Binary {_enc}", 'zh_CN': f"二进制 {_enc}"}
	translations.ids = translations._get_ids()
	translations.translations = translations._get_translations()
	bpy.app.translations.register(__name__,translations.translations)

	# 强制启用简体中文界面翻译，确保汉化一定生效（不改全局语言则插件仍显示英文）
	try:
		_ui = bpy.context.preferences.view
		_ui.use_translate_interface = True
		_ui.use_translate_new_dataname = True
		# Blender 4.5+ 用 zh_HANS 表示简体中文，旧版用 zh_CN，逐个尝试可用代码
		for _code in ('zh_HANS', 'zh_CN'):
			try:
				if _ui.language != _code:
					_ui.language = _code
				break
			except Exception:
				continue
